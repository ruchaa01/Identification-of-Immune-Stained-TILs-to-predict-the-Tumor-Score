...
%% IMAGE READ
dataset_length= 800;
imgFolder=fullfile('C:\Users\Rucha Apte\Desktop\BE Project\Training Dataset');
img_dataset=imageSet(imgFolder);
Feature_Vector_Final=[];
for k= 1:800
%% ROLLING BALL FOR IMAGE ENHANCEMENT
Img=read(img_dataset, k);
m=size(Img,1);
n=size(Img,2);
...
%% TABLE TO CSV
writetable(Feature_Vector_Final,'TRAINING DATA.csv')
end
timeElapsed = toc;
disp(timeElapsed)